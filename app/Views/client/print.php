                        ...
                        <div class="card-body">
                            <div class="table-responsive table table-bordered">
                                <table id="clientTable" class="table">
                                    <thead>
                                        <tr>
                                        <th style="width: 250px;">RegistrationNo</th>
                                            <th style="width: 300px;">Fullname</th>
                                            <th style="width: 250px;">Brand</th>
                                            <th style="width: 250px;">Model</th>
                                            <th style="width: 200px;">EngineCapacity</th>
                                            <th style="width: 150px;">Horsepower</th>
                                        ...
                                        <?php if (!empty($clientData)): ?>
                                            <?php foreach ($clientData as $Client): ?>
                                                ...
                                                <td><?= $Client->RegistrationNo; ?></td>
                                                    ...
                                                    <td>
                                                        <a class="btn btn-info view-files-btn"
                                                            href="<?= base_url('client/printCOR/' . md5($Client->RegistrationNo . 'View Files')); ?>"
                                                            target="_blank">
                                                            <i class="fas fa-certificate"></i> Print COR
                                                        </a>
                                                     <!--    <a class="btn btn-info view-files-btn"
                                                            href="<?= base_url('client/printOC/' . md5($Client->RegistrationNo)); ?>"
                                                            target="_blank">
                                                            <i class="fas fa-receipt"></i> Print OC
                                                        </a> -->
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
...
<script>
    ...
    //Print OC
    $(document).on('click', '.print-oc-btn', function(e) {
    e.preventDefault();

    let registrationNo = $(this).data('registration-no');
    console.log('Sending RegistrationNo:', registrationNo);

    $.ajax({
        url: `<?= base_url('client/printOC'); ?>/${registrationNo}`,
        method: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response.status === 'success') {
                // Redirect to the new printable view
                window.location.href = `<?= base_url('client/printOCView'); ?>/${registrationNo}`;
            } else {
                alert(response.message);
            }
        },
        error: function() {
            alert('An error occurred while fetching the payment data.');
        }
    });
});
